...
df <- read_excel(path = here("Code", "Annual Report", "Materials", "tabn219.65line_clean.xlsx"),
                 sheet = "processed") 
...
tf$nudge_y <- ifelse(tf$Category %in% "White", 1.2,
                     ifelse(tf$Category %in% c("Black"),-2.5, -2))
tf$nudge_x <- 150
...
yAxisBreaks <- seq(0, 100, by = 20)
yAxisLabels <- yAxisBreaks %>% ifelse(. %in% c("100"), "100%", .)
...
saveGIF({
  ...
  for (i in 1:max(tf$.frame)) {
    ...
    g <- ggplot(data = subset(tf, .frame <= i), aes(x = Year, y = Value, .frame = i)) +
      ...
      scale_y_continuous(labels=yAxisLabels, expand = c(0, 2), breaks=yAxisBreaks,limits = yAxisLimits) +
      theme_minimal() + theme_white + 
      scale_color_manual(values=cols) + scale_fill_manual(values=cols) +
...
movie.name=here("Code", "Annual Report", "Results", "tabn219.65line.gif"),interval = .02, ani.width = 1200, ani.height = 800)
...
gif_compress("/Users/Yuqi/Desktop/Files/AIR/GIT/InternationalAssessment_DataViz/Code/Annual\\ Report/Results/tabn219.65line.gif","/Users/Yuqi/Desktop/Files/AIR/GIT/InternationalAssessment_DataViz/Code/Annual\\ Report/Results/tabn219.65line_compressed.gif")
