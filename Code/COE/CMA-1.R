...
df <- read_excel(path = here("Code", "COE", "Materials", "CMA-1_cleanData.xlsx"),
                 sheet = "processed") %>% 
...
tf$nudge_y <- ifelse(tf$Category %in% "State", 25,
                     ifelse(tf$Category %in% c("Local"),-25,0))
...
plotTitle <- c("Revenues for public elementary and secondary schools, by\nrevenue source: School years 2009–10 through 2017–18")
...
theme_white <- theme(text = element_text(family="PublicoText-Roman", color = "black"),
                     ...
                     plot.margin = margin(t = 0,r = 120, b = 0, l = 0, unit = "pt") #make enough white space to the right for geom_text labels
)
...
xAxisLabels <- paste0(year(unique(df$Year)), "—", 
                      ...
                      ifelse(. %in% c("2009—10", "2014—15", "2017—18"), ., "")
  


# y axis break labels and levels
yAxisBreaks <- seq(0, 800, by = 200)
yAxisLabels <- paste0(format(seq(0, 800, by = 200), big.mark = ","))
...
saveGIF({
  ...
  for (i in 1:max(tf$.frame)) {
    ...
    g <- ggplot(data = subset(tf, .frame <= i), aes(x = Year, y = Value, .frame = i)) +
      ...
      scale_x_date(labels=xAxisLabels, expand = c(0.01, 0), breaks=xAxisBreaks,limits =as.Date(c("2009-01-01", "2017-06-28"))) +                           
      scale_y_continuous(labels=yAxisLabels, expand = c(0, 0), breaks=yAxisBreaks,limits = yAxisLimits) +
      theme_minimal() + theme_white + 
      scale_color_manual(values=cols) + scale_fill_manual(values=cols) +
    ...
    if(i!=1){
      g <- g +
        geom_text(data = subset(tf, .frame == i), aes(label = Category), hjust=0, vjust=0.5, show.legend = FALSE, parse=TRUE, size = 7, nudge_x = 40, nudge_y = subset(tf, .frame == i)$nudge_y, family = "PublicoText-Roman") +
...
movie.name=here("Code", "COE", "Results", "CMA-1_v4.gif"),interval = .02, ani.width = 1200, ani.height = 800)
...
gif_compress("/Users/Yuqi/Desktop/Files/AIR/GIT/InternationalAssessment_DataViz/Code/COE/Results/CMA-1_v4.gif","/Users/Yuqi/Desktop/Files/AIR/GIT/InternationalAssessment_DataViz/Code/COE/Results/CMA-1_v4_compressed.gif")
