...
# df$labelNormal <- ifelse(df$Percent <= pushAsideThreshold, "", df$labelRoundToZero)
# df$labelNudge <- ifelse(df$Percent > pushAsideThreshold, "", df$labelRoundToZero)

# fix caption
# wrap function
# getWrappedText <- function(text, width, ps, dpi = showtext_opts()$dpi, cex = NULL) {
#   print(width)
#   print(dpi)
#   ops <- par("ps")
#   par(ps = ps)
#   n <- nchar(text)
#   print(paste("wrappeds:",strwidth(text, family = "Calibri", units = "inches", cex = cex)))
#   print(par('cex'))
#   for(i in n:1) {
#     wrappeds <- paste0(strwrap(text, i), collapse = "\n")
#     if(strwidth(wrappeds, family = "Calibri", units = "inches") < width/dpi) break
#   }
#   par(ps = ops) # reset par
#   wrappeds
# }
...
# specify the pathway and name of the gif output, as well as the interval, width, and height
movie.name="experiment_v12.gif",interval = .02, ani.width = 1080, ani.height = 900)
